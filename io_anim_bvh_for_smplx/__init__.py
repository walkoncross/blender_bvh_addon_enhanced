...
class BVH_SMPLX_PT_import_main(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # obj_list = context.selectable_objects
        # items = tuple()
        # default_arm = ''
        # for ob in obj_list:
        #     if ob and ob.type == 'ARMATURE':
        #         items += ((ob.name, ob.name, ''),)
        #         if not default_arm:
        #             default_arm =  ob.name

        # print('items: \n', items)
        # operator.target = EnumProperty(
        #     items=items,
        #     name="Target",
        #     description="Available armatures",
        #     default=default_arm,
        # )

        ob = context.object
        if ob and ob.type == 'ARMATURE':
            operator.target = ob.name
